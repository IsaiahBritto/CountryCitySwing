...
export async function GET(req: NextRequest) {
  ...
  let query = supabaseServer
    .from("competitions")
    .select(
      "*, event:events(id, title, starts_at, ends_at, time_zone, type), entries:comp_entries(count), judges:comp_judge_assignments!comp_judge_assignments_competition_id_fkey(count), rounds:comp_rounds(count)"
    )
