...
import { postHeartbeat } from "@/lib/spotify/djSessionServer";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await postHeartbeat({
      ...
      playbackSnapshot: parsePlaybackSnapshot(body.playbackSnapshot),
    });
    ...
    if (result === "NOT_HOST") {
      return NextResponse.json(
        { error: "Only the playback host may send heartbeats" },
        { status: 403 }
      );
    }
    ...
    console.error("DJ session heartbeat error:", error);
    const message =
      error instanceof Error ? error.message : "Failed to update heartbeat";
