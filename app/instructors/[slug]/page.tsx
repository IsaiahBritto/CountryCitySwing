...
import InstructorProfileClient from "@/app/team/[slug]/InstructorProfileClient";
...
/**
 * Profile page for instructors reached from the Find Instructors directory
 * (includes both CCS team and non-CCS directory instructors).
 */
export default async function InstructorProfileBySlugPage({
  params,
}: {
  ...
  const nameParts = slugToName(decodedSlug);
  if (!nameParts) {
    return <NotFound />;
  }
  ...
  return <InstructorProfileClient profile={matchingProfile} fromDirectory />;
}
