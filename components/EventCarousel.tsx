...
export default function EventCarousel({ events }: EventCarouselProps) {
  ...
  // === 🧠 FILTER EVENTS ===
  // Keep events that are today or within the last 2 days (remove older)
  const filteredEvents = events.filter((e) =>
    dayjs(e.date).isAfter(dayjs().subtract(2, "day"), "day")
  );
  ...
  const next = () =>
    setCurrentIndex((i) =>
      i === filteredEvents.length - 1 ? 0 : i + 1
    );
  const prev = () =>
    setCurrentIndex((i) =>
      i === 0 ? filteredEvents.length - 1 : i - 1
    );
          ...
          {filteredEvents.map((event) => (
            ...
            >
              ...
              <div className="bg-neutral-800 border border-neutral-700 rounded-lg p-6 text-left shadow-lg hover:shadow-glow transition-all w-full max-w-[600px]">
                ...
                <p className="text-gray-400 italic mb-4">📍 {event.location}</p>
